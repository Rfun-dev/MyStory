...
class MainViewModelTest {
    ...
    fun `when get story should not null and return data`() = runTest {
        ...
        `when`(storyRepository.getListStory(dummyToken)).thenReturn(expectedQuote)
        val actualStory = mainViewModel.getListStory(dummyToken).getOrAwaitValue()
    ...
    fun `when Get Story Empty Should Return No Data`() = runTest{
        ...
        `when`(storyRepository.getListStory(
            dummyToken
        )).thenReturn(expectedStory)

        val mainViewModel = MainViewModel(storyRepository)
        val actualStory: PagingData<StoryEntity> = mainViewModel.getListStory(dummyToken).getOrAwaitValue()
